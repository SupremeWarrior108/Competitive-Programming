...
void solve() {
    ...
    sort(all(a));
    vec b(n, -1); // Room assignments
    int room_count = 0;

    priority_queue<pii, vector<pii>, greater<pii>> room;

    for (int i = 0; i < n; i++) {
        if (!room.empty() && room.top().ff < a[i].ff.ff) {
        b[a[i].ss] = room.top().ss;
        pii x = room.top();
        room.pop();
        room.push({a[i].ff.ss, x.ss});
        } else {
        b[a[i].ss] = ++room_count;
        room.push({a[i].ff.ss, room_count});
        }
    }
