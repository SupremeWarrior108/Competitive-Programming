...
using ordered_multiset = tree<pair<int, int>, null_type, less<pair<int, int>>,
...
struct ranges {
  int l, r, ind;
  bool operator<(const ranges &a) const {
    if (l == a.l)
      return r > a.r;
    return l < a.l;
  }
...
void solve() {
    ...
    // Check input size
    if (n <= 0) {
        cout << "Error: n should be greater than 0" << endl;
        return;
    }
    ...
    // First pass to mark contained intervals
    int m = 0;
    for (int i = 0; i < n; i++) {
        if (a[i].r <= m)
        b[a[i].ind] = true;
