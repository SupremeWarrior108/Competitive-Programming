// https://codedrills.io/contests/zco-2024-main/problems/vegetables
...
void solve() { 
    ...
    ll cost = 0;
    for(int i = 0; i<n; i++){
        b[i] = {min(ai[i], bi[i]), max(ai[i], bi[i])};
        cost += ai[i] * bi[i];
    }
    sort(all(b), cmp);
    for(int i = 0; i<n; i++){
        a[i+1] = {a[i].ff + b[i].ff, a[i].ss + b[i].ss*b[i].ff};
    }
    while(q--){
        ll x;
        cin>>x;
        pll cur = {x, 0};
        auto it = low(all(a), cur);
        if(it == a.end())
            cout<<0<<endl;
        else{
            if(*it.ff==x)
                cout<<cost - *it.ss<<endl;
            else{
                int p = it - a.begin() - 1;
                cout<<cost - *it.ss + (*it.ff - p)*(b[i].ss)<<endl;
            }
        }
    }
}
