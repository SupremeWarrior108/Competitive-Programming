//  https:www.hackerearth.com/practice/basic-programming/implementation/basics-of-implementation/practice-problems/algorithm/stringify-2-304b51ea/?purpose=login&source=problem-page&update=google/
...
void solve() { 
    ...
    cin>>n>>a[0]>>a[1]>>st;
    a[2] = n - a[0] - a[1];
    for(int i = 0; i<3; i++)   b[i] = count(all(st), char('0' + i));
    int ans = 0;
    for(int i = 0; i<n; i++)
        if(a[0]>b[0]){
            int c=st[i]-'0';
