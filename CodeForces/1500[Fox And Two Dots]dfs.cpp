...
void solve() { 
    ...
    int dy[] = {0, 0, 1, -1};
    bool check = false;
    v<vec> visited(n, vec(m, 0));
    v<string> board(n);
    auto dfs = [&](int n, int m, int x, int y, int fromx, int fromy, char c, auto&& dfs)->void{
        if(x<0 || x>=n || y<0 || y>=m)
            return;
        if(board[x][y] != c)
            return;
