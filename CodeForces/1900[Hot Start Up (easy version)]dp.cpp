...
void solve() {
  ...
  auto f = [&](int i, ll col, auto &&f) -> ll {
    if (i == n) return 0;
    if (dp[i][col] != -1) return dp[i][col];
    ll x = f(i + 1, a[i - 1], f) + (col == a[i] ? hot[a[i]] : cold[a[i]]);
    ll y = f(i + 1, col, f) + (a[i - 1] == a[i] ? hot[a[i]] : cold[a[i]]);
    return dp[i][col] = min(x, y);
  };
  cout << cold[a[0]] + f(1, 0, f) << endl;
}
