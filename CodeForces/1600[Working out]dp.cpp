...
#define fo(i, start, end, step) for (ll i = (start); (step) > 0 ? i < (end) : i > (end); i += (step))
...
void solve() {
    int p = 1005;
    vvi a(p, vi(p, 0)), dp1(p, vi(p, 0)), dp2(p, vi(p, 0)), dp3(p, vi(p, 0)), dp4(p, vi(p, 0));
    int n, m;
    cin >> n >> m;
    ...
    for (int i = 1; i <= n; ++i) {
        for (int j = 1; j <= m; ++j) {
            dp1[i][j] = a[i][j] + max((i > 1 ? dp1[i - 1][j] : 0), (j > 1 ? dp1[i][j - 1] : 0));
        }
    }

    for (int j = m; j >= 1; --j) {
        for (int i = 1; i <= n; ++i) {
            dp2[i][j] = a[i][j] + max((i > 1 ? dp2[i - 1][j] : 0), (j < m ? dp2[i][j + 1] : 0));
        }
    }

    for (int i = n; i >= 1; --i) {
        for (int j = 1; j <= m; ++j) {
            dp3[i][j] = a[i][j] + max((i < n ? dp3[i + 1][j] : 0), (j > 1 ? dp3[i][j - 1] : 0));
        }
    }

    for (int i = n; i >= 1; --i) {
        for (int j = m; j >= 1; --j) {
            dp4[i][j] = a[i][j] + max((i < n ? dp4[i + 1][j] : 0), (j < m ? dp4[i][j + 1] : 0));
        }
    }
    ...
    for (int i = 2; i < n; ++i) {
        for (int j = 2; j < m; ++j) {
            ll tot1 = dp1[i - 1][j] + dp2[i][j + 1] + dp3[i][j - 1] + dp4[i + 1][j];
            ll tot2 = dp1[i][j - 1] + dp2[i - 1][j] + dp3[i + 1][j] + dp4[i][j + 1];
