...
// Solve
vvi matrix;
vvi multiplyMatrices(const vvi &A, const vvi &B, int n) {
    vvi C(n, vector<long long>(n, 0));
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            for (int k = 0; k < n; ++k) {
...
vvi identityMatrix(int n) {
    vvi I(n, vector<long long>(n, 0));
    for (int i = 0; i < n; ++i) {
        I[i][i] = 1;
    }
    return I;
...
vvi matrixExponentiation(vvi M, long long n, int size) {
    vvi result = identityMatrix(size);
    while (n > 0) {
        if (n % 2 == 1) {
            result = multiplyMatrices(result, M, size);
        }
        M = multiplyMatrices(M, M, size);
        n /= 2;
    }
    return result;
}
...
void solve() {
    ...
    matrix.resize(m, vector<ll>(m));
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < m; j++) {
            matrix[i][j] = (s[i] * s[j] + s[i] * l[j] + l[i] * s[j]) % mod;
        }
    }
    vvi result = matrixExponentiation(matrix, n - 1, m);
 
    vector<int> M1(m);
    for (int i = 0; i < m; i++) {
        M1[i] = (s[0] * s[i] + s[0] * l[i] + l[0] * s[i]) % mod;
    }
    ll ans = 0;
    for (int i = 0; i < m; i++) {
        for (int j = 0; j < m; j++) {
            ans = (ans + M1[i] * result[i][j]) % mod;
        }
    }
    cout << ans;
}
